...
namespace IUP.Toolkits.CoroutineShells
{
    ...
    public sealed class CoroutineShell<T1, T2> : BaseCoroutineShell
    {
        ...
        public CoroutineShell(MonoBehaviour performer, Func<T1, T2, IEnumerator> routine)
        {
            RoutineShell.SetPerformer(performer);
            _routine = routine;
        }

        private Func<T1, T2, IEnumerator> _routine;
        ...
        /// </summary>
        /// <param name="routine">Корутина.</param>
        public void SetCoroutine(Func<T1, T2, IEnumerator> routine)
        {
            if (IsStarted)
            {
        ...
        {
            RoutineShell.Start(_routine(arg1, arg2));
        }
        ...
        {
            RoutineShell.StartAnyway(_routine(arg1, arg2));
        }
    }
}
